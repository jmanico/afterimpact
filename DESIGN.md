...
- Primary audience: A person recovering from a recent car accident (the accident victim).
...
AfterImpact is the known product name. Its brand direction and personality are TO BE DECIDED, so the current system is a neutral, readable provisional foundation rather than a claim about tone.
...
Approved text/background pairs and WCAG contrast ratios are: `text` on `background` 15.45:1; `text` on `surface` 16.45:1; white on `primary` 8.77:1; white on `secondary` 5.47:1; white on `error` 6.57:1; white on `success` 6.45:1; and `primary` links on `background` / `surface` 8.24:1 / 8.77:1. Use only these documented pairings for text. The `logo.svg` colors are `primary` and `secondary`.

## Typography

The broader typography direction is TO BE DECIDED. The provisional primary family is `system-ui`, with `-apple-system`, `BlinkMacSystemFont`, `"Segoe UI"`, and `sans-serif` fallbacks. It is neutral, readable, requires no network or font license dependency, and follows the user's platform conventions.
...
Provisional responsive breakpoints are `sm` 30rem, `md` 48rem, `lg` 64rem, and `xl` 80rem. They are implementation-neutral starting points and must be revisited when platform targets are decided. Layouts should reflow rather than depend on a particular device size.
