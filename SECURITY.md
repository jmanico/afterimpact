...
- **Authentication model:** Passkey-only (FR-1.2, D-9), delivered through the managed identity service (`ARCHITECTURE.md`) with password authenticators disabled; there is no shared-secret credential to protect (SEC-AUTHN-9).
- **Trust boundaries:** The four boundaries in `ARCHITECTURE.md` **Trust boundaries**, referred to below as boundaries 1, 2, 3, and 4. Boundary 4 (system ↔ managed identity provider) is governed by SEC-SESSION-3 and the passkey-only constraint recorded in `ARCHITECTURE.md`'s Technology Decisions.
...
- **SEC-AUTHN-1** Authentication MUST use passkeys (WebAuthn) (FR-1.2). Registration and assertion ceremonies are performed by the managed identity provider (`ARCHITECTURE.md`), which MUST validate each ceremony fully server-side — challenge freshness and single use, origin and RP ID binding, signature; the API application MUST NOT accept client-asserted authentication results and accepts identity only through the server-side OIDC exchange (SEC-SESSION-3). These properties are verified against the identity-provider integration configuration.
...
- **SEC-AUTHN-6** The account-recovery flow required by FR-1.6 uses recovery codes (resolves SQ-2): one-time printable recovery codes are issued at registration and regenerable on demand by the signed-in user. Recovery MUST require a valid recovery code plus confirmation via the verified email address; completing recovery MUST invalidate all existing sessions, MUST conclude with registration of a new passkey, and MUST send notice to the account email (SEC-AUTHN-11). Recovery responses MUST be enumeration-safe and timing-equivalent (SEC-AUTHN-5, T-9), and recovery mail MUST honor SEC-EXT-4. The flow provides no persistent authentication path weaker than the primary passkey flow and does not rely on emailed codes alone as an authentication factor; the code half functions during an email-provider outage.
  - **Applies to:** Identity & session handling
  - **Verification:** Design review against `REF-63B-4`; session-invalidation test on recovery completion; enumeration- and timing-equivalence tests on the recovery flow
...
- **SEC-SESSION-3** Sessions use the backend-for-frontend pattern (resolves SQ-3): the API exchanges the managed identity service's OIDC authorization code server-side and issues an opaque session identifier; identity-service tokens MUST never reach the browser. For the web client the session identifier MUST be carried in an HttpOnly, Secure, SameSite=Lax cookie with CSRF defenses applied (`PRM-CSRF`); mobile clients store the session token per SEC-SESSION-6. Session identifiers MUST be opaque, generated with a cryptographically secure random source, transmitted only over TLS, never placed in URLs, and never written to logs (NFR-1.7). This rule governs trust boundary 4 (system ↔ identity provider).
  - **Applies to:** Client ↔ API boundary
  - **Verification:** Cookie-attribute and CSRF tests; token-entropy and log-scan checks; test that no identity-service token is exposed to the browser
...
- **SEC-EXT-2** Emailed action links (verification, and the recovery-confirmation mail of the SEC-AUTHN-6 flow) MUST be single-use and time-limited, and their token values MUST be stored server-side only as one-way hashes — the usable value existing solely in the outbound email — so database or backup read access cannot redeem a live link (T-6). An unverified account and its verification link MUST expire together; a registration attempt against an already-registered address MUST behave enumeration-safely (SEC-AUTHN-5) and MUST NOT create a second bindable account; and verification mail MUST state that inaction cancels the request (T-5). The verification-link lifetime for FR-1.1 is 24 hours; the unverified account expires with the link.
...
|---|---|---|---|---|---|
| T-1 | STRIDE: DoS / Attack-path | Identity & session handling (FR-1.4 lockout) | Anyone who knows the victim's email drives 5 failed ceremonies per 15 min to renew the account-scoped block indefinitely, locking the owner out of evidence at a claim deadline; passkey-only makes the lockout near-worthless for brute force but a reliable renewable DoS. | FR-1.4, SEC-AUTHN-4, SEC-AUTHN-5, SEC-HTTP-6, SEC-LOG-1, SEC-LOG-3 | Lockout was account-scoped only, with no source throttling/anti-automation and no cap on consecutive locks, and the docs were silent on whether a valid ceremony bypasses the block. Resolved: FR-1.4 clarified (a valid passkey ceremony is not blocked); SEC-AUTHN-4 requires source-based throttling ahead of the account block; SEC-HTTP-6 records the limit values (SQ-9 resolved). **High** · resolved |
| T-2 | STRIDE: Spoofing/DoS / Takeover | Identity & session handling — account recovery (FR-1.6) | While SQ-2 was open: (a) an email-anchored recovery flow lets an attacker holding the mailbox seize the whole account and suppress notices in the same inbox; (b) a user who loses every passkey has no flow and permanently loses irreplaceable records — D-9's acknowledged cost. | SEC-AUTHN-6, SEC-EXT-2, SEC-SESSION-5, SEC-AUTHN-5, SEC-AUTHN-8, FR-1.6, FR-1.2, D-9 | Resolved: SEC-AUTHN-6 defines the recovery-codes flow (code + email confirmation, all-session invalidation, new passkey registration, notice to the account email) — the code is the factor beyond mailbox control and works during a mail outage; SEC-AUTHN-8 prompts registration of a second passkey while only one exists (SQ-2 resolved). **High** · resolved |
...
| T-5 | STRIDE: Spoofing | Identity & session handling — registration + email verification (FR-1.1) | A hostile party or automated abuser pre-registers the victim's email bound to the attacker's passkey; if the stressed victim later clicks the verification mail, a verified account under their email exists on the attacker's credential, poisoning future email-anchored recovery. | SEC-AUTHN-3, SEC-AUTHN-5, SEC-EXT-2 | Resolved: **SEC-EXT-2** — unverified accounts expire with their link, re-registration is enumeration-safe and creates no second bindable account, mail states inaction cancels, and the link lifetime is 24 hours. **Medium** · resolved |
| T-6 | STRIDE: Information Disclosure | Identity & session handling; Data persistence | A malicious/compromised operator or anyone with DB or backup read access reads stored verification/recovery token values and redeems a live email-change or (eventual SQ-2) recovery link without touching the mailbox — nothing forbade plaintext server-side token storage. | SEC-EXT-2, SEC-DATA-1, SEC-TRUST-2, SEC-INPUT-2 | Resolved: **SEC-EXT-2** — emailed token values stored server-side only as one-way hashes; plaintext exists solely in the mail; the SQ-2/SQ-5 dependencies are resolved. **Medium** · resolved |
| T-7 | STRIDE: Spoofing/Tampering | Trust boundary 1 — session token mechanics (SQ-3) | If bearer tokens were chosen and stored script-readable, any web-client injection would exfiltrate the token for replay for up to FR-1.10's absolute lifetime. If cookies are chosen, CSRF applies. | SEC-SESSION-1, SEC-SESSION-2, SEC-SESSION-3, SEC-HTTP-1, SEC-HTTP-2, SEC-HTTP-4, SEC-OUT-1 | Resolved: SEC-SESSION-3's BFF cookie model keeps tokens out of script reach (HttpOnly cookie; identity-service tokens never reach the browser), CSRF defenses apply, and FR-1.10's tightened lifetimes bound replay (SQ-3/SQ-4/CQ-2 and FR-1.10 resolved). **Medium** · resolved |
...
| T-29 | STRIDE/LINDDUN: Disclosure/Linkability | Trust boundary 3; Outbound email boundary | A compromised/malicious email provider, a network interceptor, or a mailbox-controlling household adversary reads everything crossing boundary 3; had email content admitted appointment purpose or provider names, health conditions would leak, and even minimal content lets message timing link the address to accident-case management. Mailbox copies outlive FR-9.3 deletion. | SEC-DATA-6, SEC-DATA-3, SEC-EXT-1, SEC-EXT-2, SEC-EXT-3, SEC-AUTHN-6, SEC-AUTHN-9, SEC-SESSION-5, FR-5.6, D-7, D-9 | Resolved: SEC-DATA-6 mandates minimal-content mail (SQ-12); SEC-EXT-3 selects AWS SES with TLS transport and the AWS GDPR Data Processing Addendum. Residual: mailbox copies still outlive FR-9.3 deletion and message timing still links the address to case management — inherent to the email channel, accepted. **Medium** · resolved |
| T-30 | STRIDE: Denial of Service | Outbound email boundary; Background processing (FR-5.6) | Automated abuse or a scripted hijacked session drives registration/email-change against arbitrary addresses or creates unbounded reminders, mail-bombing targets and burning sender reputation until the domain is throttled; independently, a provider outage removes the only external channel (verification, recovery, notices, deadline reminders) with no fallback. | SEC-HTTP-6, FR-5.6, NFR-3.1 | Resolved: SEC-HTTP-6's per-recipient email hard cap is a MUST (SQ-9); SEC-EXT-4 surfaces persistent delivery failure in-app; the SEC-AUTHN-6 recovery-code half works during a mail outage (SQ-2); AWS SES is the provider (SEC-EXT-3). **Medium** · resolved |
...
| SEC-HTTP-3 | NFR-1.5 (supporting) | Web client (React); REST API application (Quarkus/Kotlin) | CONFIRMED |
...
| SEC-FILE-2 | NFR-1.5, FR-3.5 | REST API application (Quarkus/Kotlin); Web client (React) | CONFIRMED |
...
| SEC-OUT-1 | NFR-1.5 (client-side complement) | Web client (React); Mobile client (Compose MP) | PROVISIONAL |
| SEC-OUT-2 | FR-7.6, FR-9.1, FR-9.2 | REST API application (Quarkus/Kotlin); Background processing | PROVISIONAL |
| SEC-OUT-3 | FR-6.1 (contact fields, conditional) | Web client (React); Mobile client (Compose MP) | PROVISIONAL |
...
| SEC-DATA-3 | **NFR-2.1** | Web client (React); Mobile client (Compose MP); REST API application (Quarkus/Kotlin) | CONFIRMED |
...
| SEC-DATA-7 | **NFR-2.2** | Web client (React); Mobile client (Compose MP); REST API application (Quarkus/Kotlin) | CONFIRMED |
...
| SEC-SECRET-1 | NFR-1.7 (extension) | All server components; Web client (React); Mobile client (Compose MP) | CONFIRMED |
...
| SEC-LOG-3 | NFR-1.6 (T-27) | REST API application (Quarkus/Kotlin); Web client (React); Mobile client (Compose MP) | CONFIRMED |
...
| SEC-ERR-1 | NFR-5.3, FR-1.3, FR-1.9 (disclosure) | REST API application (Quarkus/Kotlin); Web client (React); Mobile client (Compose MP) | CONFIRMED |
...
| SEC-EXT-4 | FR-1.1, FR-1.6, FR-5.6 (T-28, T-30, T-49) | Outbound email boundary (external); Background processing; Web client (React); Mobile client (Compose MP) | CONFIRMED |
...
- **SQ-2** — Resolved: recovery-codes flow. Recorded in SEC-AUTHN-6 (with SEC-AUTHN-8's second-passkey prompt and SEC-EXT-2's recovery-mail rules).
- **SQ-3** — Resolved: backend-for-frontend cookie sessions. Recorded in SEC-SESSION-3.
- **SQ-4** — Resolved: the web client is a separate React implementation (CQ-2, `ARCHITECTURE.md`). Header/CSP set recorded in SEC-HTTP-3; file delivery in SEC-FILE-2.
